...
struct iso2022 {
    ...
    bool (*from_ucs)(long int ucs, int *subcharset, unsigned long *bytes);
};
...
static bool write_iso2022s(charset_spec const *charset, long int input_chr,
                           charset_state *state,
                           void (*emit)(void *ctx, long int output),
                           void *emitctx)
{
...
static bool iso2022jp_from_ucs(long int ucs, int *subcharset,
			      unsigned long *bytes)
{
    int r, c;
    if (ucs < 0x80) {
...
static bool iso2022kr_from_ucs(long int ucs, int *subcharset,
                               unsigned long *bytes)
{
    int r, c;
    if (ucs < 0x80) {
