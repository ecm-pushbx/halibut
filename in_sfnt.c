...
static char *sfnt_psname(font_info *fi, errorstate *es) {
    sfnt *sf = fi->fontfile;
    t_name name;
    void *ptr, *end;
    size_t i;
	...
	err_sfntnotable(es, &sf->pos, "name");
	return NULL;
    }
...
static void sfnt_mapglyphs(font_info *fi, psdata *psd, errorstate *es) {
	...
	switch (post.format) {
	  ...
	  case 0x00030000:
	    break;
	  default:
	    err_sfnttablevers(es, &sf->pos, "post");
	    break;
	}
    }
  noglyphs:
 ...
 */
void sfnt_getmetrics(font_info *fi, errorstate *es) {
    sfnt *sf = fi->fontfile;
    t_hhea hhea;
    t_OS_2 OS_2;
    void *ptr, *end;
	...
	err_sfntbadtable(es, &sf->pos, "hmtx");
	return;
    }
    for (i = 0; i < sf->nglyphs; i++) {
    ...
    return;
  bados2:
    err_sfntbadtable(es, &sf->pos, "OS/2");
}
 ...
 * everything else.
 */
static void sfnt_getkern(font_info *fi, errorstate *es) {
    sfnt *sf = fi->fontfile;
    t_kern kern;
    ...
    return;
  bad:
    err_sfntbadtable(es, &sf->pos, "kern");
    return;
}
 ...
 */
static void sfnt_getmap(font_info *fi, errorstate *es) {
    sfnt *sf = fi->fontfile;
    t_cmap cmap;
    encodingrec *esd;
    void *base, *ptr, *end;
...
void sfnt_writeps(font_info const *fi, FILE *ofp, psdata *psd, errorstate *es) {
    ...
    return;
  badloca:
    err_sfntbadtable(es, &sf->pos, "loca");
}
